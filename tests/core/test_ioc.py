...
def test_ioc(db_path):
    ...
    ioc.register_callable(MockSqliteCursor, mock_make_sqlite_cursor)
    ioc.register_callable(MockRepo, MockSqliteRepo)
    ioc.register_callable(MockFeature, MockFeature)
    ...
    ioc.register_constant("db_path", new_path)
    repo = ioc.resolve(MockRepo)
...
def test_ioc_cache(db_path):
    ...
    ioc.register_callable(MockSqliteCursor, mock_make_sqlite_cursor, cache=True)
    cursor_1 = ioc.resolve(MockSqliteCursor)
    cursor_2 = ioc.resolve(MockSqliteCursor)
    assert cursor_1 == cursor_2
